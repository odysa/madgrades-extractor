...
public class TermReports {
    ...
    private final Table<String, Integer, String> courseDescriptions = HashBasedTable.create();

    private final Table<String, Integer, FullAttributes> courseAttributes = HashBasedTable.create();

    public Term getOrCreateTerm(int termCode) {
        return terms.computeIfAbsent(termCode, Term::new);
    }

    private void setCourseDescriptions(Course course) {
        for (Subject subject : course.subjects()) {
            if (subject.getAbbreviation() == null) {
                continue;
            }
            var row = courseDescriptions.row(subject.getAbbreviation());
            var desc = row.get(course.getCourseNumber());
            if (desc != null) {
    ...
    private void setAttributes(Course course) {
        if (course.getName().isPresent()) {
            var row = courseAttributes.row(course.getName().get());
            var attributes = row.get(course.getCourseNumber());
            if (attributes != null) {
    ...
    public void setCourseAttributes(String name, int courseNumber, FullAttributes attributes) {
        courseAttributes.put(name, courseNumber, attributes);
    }

    public void setCourseDescriptions(String abbr, int courseNumber, String desc) {
        courseDescriptions.put(abbr, courseNumber, desc);
    }
}
